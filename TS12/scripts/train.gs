...
//! This class is used privately and internally by Train and is not a %Trainz API class intended for general use.
//
// <b WARNING: THIS CLASS IS NOT TO BE USED OR RELIED ON.>
//
final class JunctionPermitInfo
{
	// PRIVATE IMPLEMENTATION
	// DO NOT USE OR RELY ON THESE MEMBERS
...
//
// <l Message  Messages> sent/received by a Train game object are:
//
...
{
	//! \name   Train Control Modes
	//  \anchor ctrlFlags
	//@{
	//! Values that specify the modes of control a train can be in.
	//
	// By setting the control mode of a train with <l Train::SetAutopilotMode  SetAutopilotMode>(),
	// the user's ability to control the train can be restricted to various levels.
	//
	// See Also:
	//     Train::GetAutopilotMode(), Train::GetAllowsUserControl(), Train::SetAutopilotMode()
	//
	...
	//  \anchor physMode
	//@{
	//! Specifies the physics mode of a train.
	//
	// Note: 
	//     As <l Train::SetPhysicsMode  SetPhysicsMode>() never worked and isn't supported, these
	//     constants are now redundant.
	//
	// See Also: 
	//     Train::GetPhysicsModel(), Train::SetPhysicsMode() 
	//
	...
	//! \name   Reverser Traction Modes
	//  \anchor revTract
	//@{
	//! Traction modes that a train's reverser lever can be in.
	//
	// See Also:
	//     Train::SetReverser(), Train::Turnaround()
	//
	...
	//! \name   Train Brake Modes
	//  \anchor trainBrakes
	//@{
	//! Specifies the state of the train brakes.
	//
	// See Also:
	//     Train::SetTrainBrakes()
	//
	...
	//! \name   Dynamic Brake States
	//  \anchor dynamicBrakes
	//@{
	//! States the dynamic brakes of a train can be in.
	//
	// See Also:
	//     Train::SetDynamicBrakeMode() 
	//
	...
	//  \anchor foundRes
	//@{
	//! Results codes for a search indicating the found sate.
	//
	// See Also:
	//     Train::GetTrackmarkDirection()
	//
	...
	//! \name   Measurement Conversion Multipliers
	//  \anchor measureConv
	//@{
	//! Measurement conversion multiplier values.
	//
	// See Also:
	//     Train::GetTrainVelocity(), Vehicle::GetVelocity()
	//

	public define float KPH_TO_MPS = 0.278;  //!< KPH (kilometers per hour) to MPS (meters per second) conversion multiplier.
	public define float MPH_TO_MPS = 0.447;  //!< MPH (miles per hour) to MPS (meters per second) conversion multiplier.
	public define float MPH_TO_KPH = 1.609;  //!< MPH (miles per hour) to KPG (kilometers per hour) conversion multiplier.
  ...
  bool expandSchedule = false;
	...
	//! Gets the vehicles in this train.
	//
	// Note:
	//     Index 0 of the returned vehicles array is always the first vehicle of the train, which is
	//     dependent on the train's forward direction.  The forward direction of the train can be
	//     changed during play and may not necessarily correspond to the train's visual appearance.
	//
	// Returns:
	//     Returns an array of vehicles that define this train.
	//
	public native Vehicle[] GetVehicles(void);

	//! Gets the front most locomotive in this train.
	//
	// Returns:
	//     Returns the front most locomotive in this train.
	//
  ...
  // Note: During multiplayer games this function will only succeed on the server
	...
	//! Obsolete, do not use.
	public void SetAdhesion(float adhesion) { Interface.WarnObsolete("Adhesion values are no longer used in Trainz EE"); }
	...
	//! Gets the on/off state of the headlights in this train. 
	//
	// Note: 
	...
	//
	// Returns:
	//     Returns true if the headlights of this train are switched on, false otherwise. 
	//
	public native bool GetHeadlightState(void);

	//! Switch the headlights of this train on or off. 
	//
	// Note: 
	//     When the headlights of a train are switched on, only the front locomotive's headlights will
	//     actually be visibly switched on and affect the world in %Trainz.  Other locomotives in the
	//     consist are considered to have their headlights switched on in a sense.  Not only is it
	//     more prototypical to have only the front locomotive's headlights on, but this also cuts
	//     back on unnecessary lighting calculations during rendering.
	//
	// Param:  lightsOn  State to set headlights of this train to.  Use true to switch headlights on
	//                   or false to switch them off. 
	//
	public native void SetHeadlightState(bool lightsOn);

  //! Returns true if any vehicle in this train has lights
  //
  public native bool HasLights();

	//! Gets the state of the ditchlights in this train. 
	//
	// Returns:
	//     Returns true if the ditchlights of this train are flashing, false otherwise. 
	//
	public native bool GetDitchlightState(void);

	//! Flash the ditchlights of this train. 
	//
	// Param:  lightsFlashing  State to set ditchlights of this train to.  Use true to flash ditchlights
	//                         or false not to flash. 
	//
	public native void SetDitchlightState(bool lightsFlashing);

	//! Returns true if any vehicle in this train has ditchlights, false otherwise
	//
	public native bool HasDitchlight();

	//! Gets the high beam state of the headlight in this train. 
	//
	// Returns:
	//     Returns true if the headlight is set to high beam, false otherwise. 
	//
	public native bool GetHighbeamState(void);

	//! Set the high beam state for the headlight in this train
	//
	// Param:  highBeam  State to set the high beam of the headlight in this train to.  Use true for high/full beam,
	//                         or false for low/dip beam. 
	//
	public native void SetHighbeamState(bool highBeam);

  //! Gets the state of the pantographs of the electric locomotives in this train.
	//
	// Note:
	//     The pantographs can only be in one of four states and what these states are is dependents
	//     on the asset.  Obviously, this method will be of little use on a train that has no electric
	//     locomotives.
	//
	// Returns:
	//     Returns the current pantograph state of this train.  Will be in the range of [0 - 3].
	//
	public native int GetPantographState(void);

	//! Sets the state of the pantographs of the electric locomotives in this train.
	//
	// Note:
	//     The pantographs can only be in one of four states and what these states are is dependents
	//     on the asset.  Obviously, this method will be of little use on a train that has no electric
	//     locomotives.
	//
	// Param:  state  State to set the pantographs to.  Must be in the range of [0 - 3].
	//
	public native void SetPantographState(int state);

	//! Returns true if any vehicle in this train has pantographs
	//
  public native bool HasPantographs();


  //! Blows the horn (or whistle) for all locomotives in this train.
  //
  // See Also:
  //     AiHornMsg 
  //
  ...
  //! Returns true if any vehicle in this train has a horn
  //
  public native bool HasHorn();


	//! Gets the state of the bell for this train. 
	//
	// Returns:
	//     Returns true if the bell is ringing, false otherwise. 
	//
	public native bool GetBellState(void);

	//! Ring the bell for this train.
	//
	// Param:  bellOn  State to set bell for this train to.  Use true to ring bell
	//                         or false to silence bell. 
	//
	public native void SetBellState(bool bellOn);

	//! Returns true if any vehicle in this train has a bell sound
	//
  ...
  // Note: During multiplayer games this function will only succeed on the server and owning clients
	//
	// Param:  speed  Advisory speed limit in KPH (kilometers per hour).  Use 0 for no speed limit.
	//
	// See Also:
  ...
  JunctionBase m_trainInternalDriveToJunction;      // used internally when driving to a junction, to prevent premature permit release.
	...
	//@ Gets the state of the current schuedule.
	...
	//@ Not documented yet.
	public GameObject GetScheduleObject(void)
	{
		if (!scheduleHistory)
			return null;
		
		object obj = scheduleHistory[scheduleHistory.size() - 1].obj;
		return cast<GameObject>(obj);
	}

	//@ Pushes schedule state and saves it.
	public void PushScheduleState(AITrainScope scope, int state, object obj)
	{
  ...
  //Permit m_currentTrackPermit;      // the permit for the current bit of track we occupy
  ...
  //int[] m_junctionIds;              // the node ids of the junctions we are on
  ...
  //
	...
	public string GetScheduleStateString(void)
	{
		...
		else
		{
			...
			state = scheduleHistory[index].state;
			obj = scheduleHistory[index].obj;
		}
		
		if (state == SS_NONE)
		{
			// Whatever command we're executing has not provided any
		...
		if (state == SS_JUNCTION)
		{
			...
			return strTable.GetString1("interface-train-schedule-state-junc", junction.GetMapObject().GetLocalisedName());
		}

		if (state == SS_JUNCTION_TRACK)
		{
			JunctionBase junction = cast<JunctionBase>(obj);
			return strTable.GetString1("interface-train-schedule-state-jtrack", junction.GetMapObject().GetLocalisedName());
		}
		...
		if (state == SS_DRIVING)
		{
			...
			if (train)
				...
				// assumedly we're driving along happily
		...
		if (state == SS_VEHICLE)
		{
			...
			return strTable.GetString1("interface-train-schedule-state-vehicle", vehicle.GetLocalisedName());
		}
  ...
  public void SetHandBrake(bool state)
  {
    ...
    //AddHandler(me, "SceneryTrigger", "Enter", "SceneryTriggerHandler");
...
*/
  ...
  public thread void RunSchedule(Schedule schedule, bool loop, float delay)
  {
    ...
    logMsg = "Schedule for train " + GetId() + " ";
   ...
   run:
		...
		if (schedule.notifyDriver and !m_scheduleNotifier)
			RunScheduleNotifier();


    for(entry = 0; entry < schedule.commands.size(); entry++)
    {
      cmd = schedule.commands[entry];
      ...
      switch(cmd.command)
      {
        case Schedule.CMD_TAKE_JUNCTION :
        {
          ...
          // if(!junction) goto done;
          Interface.Log(logMsg + "CMD_TAKE_JUNCTION '" + junction.GetDebugString());
        ...
        case Schedule.CMD_TAKE_TRACK_PERMIT :
        {
          ...
          // if(!junction) goto done;
          Interface.Log(logMsg + "CMD_TAKE_TRACK_PERMIT (" + track.GetId() + ")");
          ...
          break;
				...
				case Schedule.CMD_COUPLE_VEHICLE:
				{
					...
					Interface.Log(logMsg + "CMD_COUPLE_VEHICLE (" + vehicle.GetId() + " = " + vehicle.GetName() + ")");
        ...
        {
          Interface.Log(logMsg + "CMD_STOP_AT_TRIGGER " + (cast<GameObject> cmd.obj).GetName());
          Trigger trigger = cast<Trigger> cmd.obj;
          if(!trigger) goto done;
          if(!CommandStopAtTrigger(trigger, cmd.cd)) goto done;
          break;
        }

        case Schedule.CMD_ON_TRIGGER :
        {
          Interface.Log(logMsg + "CMD_ON_TRIGGER " + (cast<GameObject> cmd.obj).GetName());
          Trigger trigger = cast<Trigger> cmd.obj;
          if(!trigger) goto done;
          if(!CommandOnTrigger(trigger, cmd.ca)) goto done;
          break;
        }

        case Schedule.CMD_STOP_AT_SCENERY :
        {
          Interface.Log(logMsg + "CMD_STOP_AT_SCENERY " + (cast<GameObject> cmd.obj).GetName());
          Buildable scenery = cast<Buildable> cmd.obj;
          if(!scenery) goto done;
          if(!CommandStopAtScenery(scenery)) goto done;
          break;
        }

        case Schedule.CMD_STOP_AT_TRACKMARK :
        {
          Interface.Log(logMsg + "CMD_STOP_AT_TRACKMARK " + (cast<GameObject> cmd.obj).GetName());
          TrackMark trackMark = cast<TrackMark> cmd.obj;
          if(!trackMark) goto done;
          if(!CommandDriveToTrackMark(trackMark, true)) goto done;
          break;
        }

        case Schedule.CMD_DRIVE_THROUGH_TRACKMARK :
        {
          Interface.Log(logMsg + "CMD_DRIVE_THROUGH_TRACKMARK " + (cast<GameObject> cmd.obj).GetName());
          TrackMark trackMark = cast<TrackMark> cmd.obj;
          if(!trackMark) goto done;
          if(!CommandDriveToTrackMark(trackMark, false)) goto done;
          break;
        }
          ...
          if(!CommandDriveForever()) goto done;
          break;
				}
        ...
        case Schedule.CMD_REVERSE_TRAIN :
        {
          ...
          if (!CommandReverseTrain(cmd.ca == 1)) goto done;
          break;
        }
        ...
        case Schedule.CMD_STOP_TRAIN :
        {
          ...
          if(!CommandStopTrain(cmd.cd)) goto done;
          break;
        }
        ...
        case Schedule.CMD_ON_ANNOUNCEMENT :
        {
          ...
          if(!CommandOnAnnouncement(cmd.minor)) goto done;
          break;
        }

        case Schedule.CMD_SET_SIGNAL :
        {
          Interface.Log(logMsg + "CMD_SET_SIGNAL " + cmd.major);
          Signal signal = cast<Signal> Router.GetGameObject(cmd.major);
          signal.SetSignalState(cmd.ca, cmd.minor);
          break;
        }
        ...
        case Schedule.CMD_CUSTOM :
        {
          ...
          if(!cmd.custom.Execute(me, cmd.ca, cmd.cb, cmd.cc)) goto done;
          break;
        }
    ...
    Router.PostMessage(GetId(), Router.MESSAGE_BROADCAST, "Schedule", "Complete", 0.1f);
  ...
  public bool IsScheduleRunning()
  {
	...
	void JunctionInnerEnterHandler(Message msg)
	{
	  Interface.Log("JunctionInnerEnterHandler");
		...
		if (m_junctionPermits)
		{
			...
			for (i = 0; i < count; i++)
				if (m_junctionPermits[i].GetObject() == msg.src)
				{
				  Interface.Log("JunctionInnerEnterHandler> already have permit for junction");
    ...
    if (junctionPermit and junctionPermit.IsGranted())
      Interface.Log("JunctionInnerEnterHandler> permit granted for junction " + junction.GetDebugString() + " (train " + GetId() + " " + GetTrainDisplayName() + ")");
    else
      Interface.Log("JunctionInnerEnterHandler> permit NOT granted on junction " + junction.GetDebugString() + " (train " + GetId() + " " + GetTrainDisplayName() + ")");
  ...
  void PermitReleaser(Message msg)
  {
    ...
    if (junction and m_junctionPermits and m_junctionPermits.size() > 0)
    {
      ...
      else
      {
				...
				while (i < m_junctionPermits.size())
        {
          if (m_junctionPermits[i].GetObject() == junction)
          {
						Interface.Log("Train.PermitReleaser> out-of-order junction permit release (" + m_junctionPermits[i].GetObject().GetId() + ")");
	...
	public bool IsStillInJunction(JunctionBase jn)
	{
		...
		int i, j = m_junctionPermits.size();
		for (i = 0; i < j; i++)
			if (m_junctionPermits[i].GetObject() == jn.GetGameObject())
				return true;
	...
	public bool HasReachedJunctionInnerRadius(JunctionBase jn)
	{
		...
		int i, j = m_junctionPermits.size();
		for (i = 0; i < j; i++)
			if (m_junctionPermits[i].GetObject() == jn)
  ...
  void ReleaseAllPermits()
  {
    ...
    if (m_junctionPermits)
		{
			...
			while (--i >= 0)
			{
				...
				if (!junction)
				{
					...
					Interface.Log(" permit has unknown object");
				}
				else
				{
          ...
          if (junction == m_trainInternalDriveToJunction)
            releasePermit = false;
						...
						releasePermit = !junction.JunctionReEnter(GetId());
            
					else
						releasePermit = !junction.JunctionInnerReEnter(GetId());
            
					Interface.Log(" releasePermit=" + releasePermit + ", movingFast=" + movingFast);
				}
				
				if (releasePermit)
				{
					Interface.Log("  releasing permit for " + m_junctionPermits[i].GetDebugString());

					m_junctionPermits[i,i+1] = null;
					m_junctionInner[i, i+1] = null;
					//m_junctionIds[i,i+1] = null;
				}
			}
		}
		else
	...
	void TrackPermitReleaser(Message msg)
	{
		...
		Interface.Log("Train.TrackPermitRelease> gameobject " + (string)(obj.GetId()));
		if (!m_trackPermits)
			return;
		...
		for (i = 0; i < m_trackPermits.size(); i++)
			...
			{
				Interface.Log("Train.TrackPermitRelease> released permit for gameobject " + (string)(obj.GetId()));
				RemoveTrackPermit(i);
	...
	{
		return cast<Industry>(lastVisitedScenery);
	}
    ...
    junction.JunctionReEnter(GetId());
    wait()
    {
	...
	bool GetJunctionTrackPermit(JunctionPermitInfo info)
	{
		...
		{
			Interface.Log("  junction.RequestTrackPermit> bad Direction");
			return false;
		}
    ...
    // get the track permit
		...
		{
			trackPermit[i] = info.junction.RequestTrackPermit(me, trackDirection[i]);
			if (trackPermit[i])
				permitCount++;
		}
		...
		if (permitCount == 0)
		{
			...
			Interface.Log(info.junction.GetDebugString() + " rejected permit(s) for train " + me.GetId());
			return false;
		}
		...
		AITrainScope scope = OpenAITrainScope();
		PushScheduleState(scope, SS_JUNCTION_TRACK, cast<object>(info.junction));
...
/*
    ...
    // get the junction permit
		Interface.Log("  junction.RequestPermit");
    ...
    {
      junctionPermit = info.junction.RequestPermit(me, info.direction);
    }
    if (!junctionPermit)
			return null;
		...
		AITrainScope scope = OpenAITrainScope();
		PushScheduleState(scope, SS_JUNCTION, cast<object>(info.junction));
		m_scheduleDrivingLocked++;
  ...
  bool CommandNavigateJunction(JunctionBase junction, int direction, bool alreadyAtJunction, bool inStuckThread)
  {
    ...
    //if (!junction.SwitchForTrain(me, info.direction))
    if (!junction.SetDirection(info.direction))
		{
			// unable to set junction!

			// we may be too far away?
			/*if (!GetAutopilotTargetSlowing())
			{
				wait()
				{
				on "Train", "SlowingForTarget":
				...
				goto retrySwitchForTrain;
			}*/

			// otherwise.. we're just stuck, somehow..
			return false;
		}
	...
	bool CommandNavigateTrack(Track track)
	{
		...
		AITrainScope scope = OpenAITrainScope();
		PushScheduleState(scope, SS_TRACK, cast<object>track);
		m_scheduleDrivingLocked ++;
  ...
  bool CommandStopAtScenery(Buildable p_scenery)
  {
      ...
      on "SceneryTrigger", "InnerEnter", msg:
      {
        if (msg.src == p_scenery)
          ...
          break;
				...
				Interface.Log("  src=" + (cast<GameObject>(msg.src)).GetName());
				Interface.Log("  didnt match scenery " + p_scenery.GetName());
        continue;
      }
	...
	bool UpdateCCJList(int[] junctionIDs, object p_src)
	{
		GameObject go = cast<GameObject>(p_src);
		int jnid = go.GetId();
		int i, j = junctionIDs.size();
		
		/*
		  THIS SEEMS TO BE A HOLDOVER FROM WHEN junctionIDs WAS A MEMBER VARIABLE
			ANYWAY IT DOESNT MAKE SENSE NOW SO I'VE REMOVED IT

		for (i = 0; i < j; i++)
			if (junctionIDs[i] == jnid)
			{
				Interface.Log("Train.UpdateCCJList> (still in use) id = " + jnid);
				return false;
			}
		*/
		...
		while (--i >= 0)
			if (junctionIDs[i] == jnid)
			{
				...
				Interface.Log("Train.UpdateCCJList> (released) id = " + jnid + ", " + junctionIDs.size() + " remain");
				return true;
			}

		Interface.Log("Train.UpdateCCJList> (not found) id = " + jnid);
		return false;
	}


	void LogCCJList(int[] junctionIDs)
	{
		string msg = "";
		
		int i, j = junctionIDs.size();
		for (i = 0; i < j; i++)
			msg = msg + " " + (string)junctionIDs[i];

		Interface.Log("  CCJ >>" + msg);
	}
	...
	bool AITrainStoppedMoving(bool force)
	{
		...
		if (IsStillInJunction(junction))
		{
			...
			if (!junction.SwitchForTrain(me, direction))
				return false;
		...
		int direction = junction.GetDirectionToTrain(me);
		if (direction == Junction.DIRECTION_BACKWARD)
			direction = 1000;//Junction.DIRECTION_NONE;//junction.GetDirection();
		else
			direction = Junction.DIRECTION_BACKWARD;
			...
			if (aiTrainStuck)
			{
				if (GetAutopilotMode() == CONTROL_AUTOPILOT)
				{
					...
					if (scheduleTimeout > 3)
					{
						if (!IsStopped())
						{
							// this is not really a good thing to have happen,
							// so lets warn about it so people will notice if it happens too often.
    ...
    Router.PostMessage(GetId(), Router.MESSAGE_BROADCAST, "Train.ThreadAITrainStuck", "Done", 0.0f);
	}
	...
	void AddJunctionPermitsToArray(Permit[] permits)
	{
		if (!permits)
			return;
		...
		for (i = 0; i < m_junctionPermits.size(); i++)
		{
			Permit permit = m_junctionPermits[i];
				...
				// ceeb040414: we don't want to force the crossing of a junction for which we have no permit
			if (!permit.IsGranted())
			{
				Interface.Log("Train.AddJunctionPermitsToArray> permit for junction " + permit.GetObject().GetId() + " is not granted- skipping");
				goto skip;
			}

			for (j = 0; j < permits.size(); j++)
				if (permits[j] == permit)
					goto skip;
			
			Interface.Log("Train.AddJunctionPermitsToArray> adding permit for junction " + permit.GetObject().GetId());
			permits[permits.size()] = permit;
	...
	/*thread void ThreadAITrainStoppedMoving(void)
	{
		if (!AITrainStoppedMoving(true))
	...
	// 
	// crossedJunctions should be passed in as an empty array (or NULL if you dont care about it)
	// it will be modified (if not NULL) to hold permits for each of the relevant junctions
	//
  ...
  bool CommandCompleteJunctions(void)
  {
    if(m_junctionPermits == null or m_junctionPermits.size() == 0)
		{
			Interface.Log("CommandCompleteJunctions> Elvis has left the building");
      return true;
		}

		Interface.Log("*** CommandCompleteJunctions> BEGIN");
		...
		if (ccjRecursionDepth++ > 0)
		{
			...
			Interface.WarnObsolete("Train.CommandCompleteJunctions> WARNING- CCJ RECURSION");
			okay = false;
			goto fail_ccj;
		}

		/*if (ccjRecursionDepth++ > 8)
		{
			// this means we're too deep in nested junctions and are probably stuck.
			// it's better to just stop the train rather than go around in circles filling up the call stack.
			// this may never happen but better safe than sorry

			Interface.Log("Train.CommandCompleteJunctions> too much recursion. aborting schedule");
		...
		int[] junctionIDs;
		int i;
		junctionIDs = new int[m_junctionPermits.size()];
		for (i = 0; i < m_junctionPermits.size(); i++)
		{
			junctionIDs[i] = m_junctionPermits[i].GetObject().GetId();
			Interface.Log("*** CommandCompleteJunctions>  jnid " + junctionIDs[i]);
		}
...
fail_ccj:
    ...
    SetDCCThrottle(0);
		...
		//
		// Since we've completed all the old junctions, and dont care about the new junctions, lets just clear
		// the inner flag on everything. This prevents NavigateJunction() from issuing another CCJ immediately.
		//
		if (m_junctionInner)
			...
			// ceeb040311: fail the command after completing junctions, we may need to re pathsearch
		//if (ccjRecursionDepth == 0)
		//	return false;
	...
	bool CommandCoupleVehicle(Vehicle vehicle)
	{
		...
		AITrainScope scope = OpenAITrainScope();
		PushScheduleState(scope, SS_VEHICLE, cast<object>vehicle);
  ...
  public void UpdateScheduleWindow(string vehicleName)
  {
    ...
    if (!expandSchedule)
      paramOutput = paramOutput + "<font color=#FFFFFF><a href=live://property/toggleExpand>" + strTable.GetString("interface-train-schedule-update0") + "</a></font><BR><BR>";
    else
    ...
    if (HasTrainTimetable())
    {
      ...
      for (i = 0; i < timetables.size(); i++)
      {
        ...
        KUID kuid;
        string icon;
        Industry industry = cast<Industry>(Router.GetGameObject(timetable.destinationName));
        TrackMark trackMark = cast<TrackMark>(Router.GetGameObject(timetable.destinationName));
        if (industry)
        {
          kuid = industry.GetAsset().GetKUID();
          icon = kuid.GetHTMLString();
        }
        if (trackMark)
        {
          Asset asset = Constructors.GetTrainzAsset();
          kuid = asset.LookupKUIDTable("drive-to-trackmark");
          icon = kuid.GetHTMLString();
        }
        ...
        paramOutput = paramOutput + "<td><font color=#FFFFFF>" + BrowserInterface.Quote(HTMLWindow.GetHumanReadableNameFromTrackName(timetable.trackName, timetable.destinationName)) + "</font></td><td width=5></td>";
    ...
    if (GetHeadlightState())
	  ...
	  viewDetailsBrowser.LoadHTMLString(paramOutput);
	}
