...
class GenericIndustry isclass Industry
{
	...
	thread void GenericIndustryMain(void)
	{
		...
		wait()
		{
			...
			on "Object", "Stopped", msg:
				...
				triggerName = FindTriggerContainingNode(vehicle.GetId(), true);
  ...
  {
    IndustryProductInfo info;
    info = new IndustryProductInfo();
    industryProductList[industryProductList.size()] = info;
    info.Init();
    info.asset = GetAsset().FindAsset(assetName);
    if (!info.asset)
      Exception("Asset could not be found (looking for: " + assetName + ")");
    info.queue = GetQueue(queueName);
    if (!info.queue)
      Exception("Queue could not be found (looking for: " + queueName + ")");
    info.isInput = isInput;
    info.processName = processName;
    info.showInViewDetails = showInViewDetails;
  }
  ...
  public string GetDescriptionHTML(void)
  {
    ...
    {
      bool exists = false;
      IndustryProcessInfo info;
      info = new IndustryProcessInfo();
      info.Init();
      ...
      if (!exists)
      {
        processes[processes.size()] = info;
        info.processName = industryProductList[i].processName;
        info.assets_consume = 0;
        info.assets_produce = 0;
      }
