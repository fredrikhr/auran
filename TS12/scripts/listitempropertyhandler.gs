...
//  string m_boolLinkTooltip = "";
  ...
  public string GetDescriptionHTML(void)
  {
//    string boolLink = "<a href='" + Link("boolValue") + "' tooltip='" + m_boolLinkTooltip + "'>";
...
//
// {[ String Name                     | Description                                                   ]
//  [ "<config-prefix>-asset-type"    | Type of asset to populate the list with assets from.  Must be an asset type
//                                      that <a class="el" href="classWorld.html#a25">World.GetAssetList</a>()
//                                      recognizes.  Some common types are <m "behavior"> (rules), <m "drivercommand">,
//                                      <m "html-asset">, <m "industry">, <m "product">, <m "product-category">,
//                                      <m "scenery"> and <m "traincar"> (vehicle).  See the <b Content Creators Guide (CCG)>
//                                      for a more complete list of the asset types Trainz supports.  ]
...
{
  string  m_assetType;                   // type of asset
  Asset[] m_assetList = new Asset[0];    // list of assets
  bool[]  m_itemSettings = new bool[0];  // settings for check boxes (checked - true, unchecked - false)
  string  m_displayName = "";            // display name/text just above the list

  public void Init(Asset asset, string newConfigPrefix);

  //! Gets the property index number to the given asset.
  //
  // This method allows the index to the bool property of the given asset to be obtained.
  //
  // Param:  asset  Asset to get the index of in this handler.
  //
  // Returns:
  //     If <i asset> is present in this property handler, the index to it is returned.  Otherwise null is
  //     returned if <i asset> is invalid or is of a different type then this handler is configured for.
  //
  ...
  public void Init(Asset asset, string newConfigPrefix)
  {
    ...
    if (!m_assetType.size())
    {
      Interface.Log("# AssetCheckListHandler.Init> ERROR: No asset type specified!");
      return;
    }

    // create the asset list
    Str.ToLower(m_assetType);
    m_assetList = World.GetAssetList(m_assetType);
    if (m_assetList.size() <= 0)
    {
      Interface.Log("# AssetCheckListHandler.Init> ERROR: No assets of type '" + m_assetType + "' found!");
      return;
    }

    // generate the accompanying check list (all unchecked by default)
    int i;
    for (i = 0; i < m_assetList.size(); i++)
      m_itemSettings[i] = false;  // defaults to false (unchecked state)
  }

  //! Gets the type of asset this handler has been setup to work with.
  //
  // This method returns the type string read from the <m "<config-prefix>-asset-type"> string table entry 
  // during initialization.  It does not verify that the type string is a valid type or anything else, 
  // just returns it.
  //
  // Returns:
  //     Returns the type of assets this config handler has been setup to work with.  If this handler is 
  //     being used correctly, the returned string will be one of the Asset <n kind> elements that %Trainz
  //     supports.
  //
  public string GetAssetType(void) { return m_assetType; }

  //! Gets the asset in this handler from the given index.
  //
  // Param:  index  Item number of Asset to get the value of.
  //
  // Note:
  //     The indexing of assets may vary across different installations, so always check the returned asset.
  //
  // Returns:
  //     Returns the asset specified by <i index> if it exists, false otherwise.
  //
  ...
  //! Gets all of the assets that are checked (or unchecked) in this handler.
  //
  // Param:  checked  State of the checkbox bool property required for an asset to be included.
  //
  // Returns:
  //     Returns an array of assets that have their checkboxes bool property set to the state specified
  //     by <i checked>.
  //
  public Asset[] GetSettingAssets(bool checked)
  {
    Asset[] resultAsset = new Asset[0];
    int i, q = 0;
    for (i = 0; i < m_assetList.size(); i++)
      if (m_itemSettings[i] == checked)
        resultAsset[q++] = m_assetList[i];
  ...
  public void SetPropertyValue(string propertyID, PropertyValue value)
  {
    ...
    else if (TrainUtil.GetUpTo(property, "/") == "itemSetting")
    {
      int itemIndex = Str.ToInt(TrainUtil.GetAfter(property, "/"));
      m_itemSettings[itemIndex] = !m_itemSettings[itemIndex];
    }
  ...
  public Soup GetProperties()
  {
    ...
    for (i = 0; i < m_itemSettings.size(); i++)
      // if the asset is checked, save its KUID to the soup (its presence will dictate that it is set)
      if (m_itemSettings[i]  and  m_assetList[i])
  ...
  public void SetProperties(Soup soup)
  {
    ...
    // Note that we assume Init() has been called and already initialized the asset array to whatever assets
    // of the required type are present and the check-list states are all false
    Soup itemsSoup = soup.GetNamedSoup("AssetCheckListHandler.items");
    int i;
    for (i = 0; i < itemsSoup.CountTags(); i++)
    {
      string itemName = itemsSoup.GetIndexedTagName(i);
      KUID itemKUID = itemsSoup.GetNamedTagAsKUID(itemName);
      if (itemKUID)
      {
        // find item location in asset array and set the equivalent bool check
        Asset asset = World.FindAsset(itemKUID);
        int assetIndex = GetAssetIndex(asset);
        if (assetIndex >= 0)
          m_itemSettings[assetIndex] = true;
      }
    }
...
//
// {[ String Name                            | Description                                                     ]
//  [ "<config-prefix>-asset-type"           | Type of asset to enumerate from.  Must be a valid Trainz asset that
//                                             <a class="el" href="classWorld.html#a25">World.GetAssetList</a>()
//                                             recognizes.  ]
...
//
// Useful interface methods provided with this class are:
//  - public string  <b GetAssetType>         ()
//  - public string  <b GetAssetName>         ()
//  - public Asset   <b GetSelectedAssetRef>  ()
//  - public Asset   <b GetAssetByIndex>      (int index)
//  - public Asset   <b GetAssetByName>       (string assetName)
//  - public void    <b SetAsset>             (Asset newAsset)
//  - public void    <b SetAssetType>         (string newType)
//
// Note:
...
{
  string m_assetType = "";  // type of asset we are dealing with (string value compataible with World.GetAssetList())
  Asset  m_selectedAsset;   // asset that is currently selected (if any). 
  ...
  //@ Gets the asset type as a string.
  //
  // Returns:
  //     ??
  //
  ...
  //@ Gets the name of the currentl selected asset, blank string otherwise.
  //
  // Returns:
  //     ??
  //
  ...
  //@ Sets the asset of handler.  Not checkedf, be careful!
  //
  // Param:  newAsset  ?? 
  //
  public void SetAsset(Asset newAsset) { m_selectedAsset = newAsset; }
  
  //@ Gets type of asset this handler deals with (as a string).
  //
  // Param:  newType  ??
  //
  public void SetAssetType(string newType) { m_assetType = newType; }
  ...
  public void Init(Asset asset, string newConfigPrefix)
  {
    ...
    if (!m_assetType.size())
    {
      Interface.Log("# EnumeratedAssetHandler.Init> ERROR: No asset type provided!");
      return;
    }

    // validate that the asset type is genuine and exists
    if (World.GetAssetList(m_assetType).size() <= 0)
      Interface.Log("# EnumeratedAssetHandler.Init> WARNING: Unable to find any assets of type '" + m_assetType + "'");
  }

  //
  // Gets the full name of the currently selected asset (if any)
  //
  ...
  //
  // Gets asset by its indexed value in the array returned by GetAssetList() for assetType
  //
  public Asset GetAssetByIndex(int index)
  {
    Asset[] assetList = World.GetAssetList(m_assetType);
    if (assetList.size())
      if ((index < assetList.size())  and  (index >= 0))
  ...
  public Asset GetAssetByName(string assetName)
  {
    ...
    if (assetList.size())
    {
      ...
      for (i = 0; i < assetList.size(); i++)
        if (assetList[i].GetLocalisedName() == assetName)
          return assetList[i];
    }
  ...
  //
  // Gets description of this property handler as a HTML page.
  //
  public string GetDescriptionHTML(void)
  {
  ...
  public void SetPropertyValue(string propertyID, PropertyValue value)
  {
    if ((PropStripPrefix(propertyID, "") == "assetName")  and  (value.GetType() == PropertyValue.INDEX_TYPE))
      m_selectedAsset = GetAssetByIndex(value.GetIndex());
    else
      inherited(propertyID, value);
  }
  ...
  public string[] GetPropertyElementList(string propertyID)
  {
    if (PropStripPrefix(propertyID, "") == "assetName")
    {
      ...
      if (assetList.size() > 0)
      {
        ...
        int i;
        for (i = 0; i < assetList.size(); i++) 
          retStrList[i] = assetList[i].GetLocalisedName();
  ...
  public void SetProperties(Soup soup)
  {
    ...
    // only load asset type from soup if it isn't already set - Init() might have loaded a newer value for
    // this from the host string-table
    if (!m_assetType.size())
...
{
  // type of game object.  Must be "behavior", "industry", "junction", "signal", "trackmark", "traincar" or "trigger"
  string m_gameObjectType = "";
  string m_gameObjectName = "";  // name of the currently selected game object
  ...
  //@ Gets the name of the currently selected object, blank string otherwise.
  //
  // Returns:
  //     Returns the name of the currently selected object, blank string otherwise.
  //
  public string GetObjectName(void) { return m_gameObjectName; }

  //@ Gets the name of the current game object if possible, 
  //
  // Gets the name of the current game object if possible, otherwise the host asset's string table entry
  // for a blank value will be returned.
  //
  // Returns:
  //     ??
  //
  ...
  //
  // Gets the current game object if possible.  As it is returned as a GameObject reference, the user will
  // need to cast it themselves (and be careful when doing so by verifying their cast object).  A message
  // will be logged and null returned if the object can't be found.
  //
  // Returns:
  //     ??
  //
  public GameObject GetGameObject(void)
  {
  ...
  {
    string property = PropStripPrefix(propertyID, "");
    if (property == "gameObjectName")
      return "list,1";

    // this generally wouldn't be used as the type isn't really meant to be changed once the handler object
    // has been initialized, so it's here for completeness really
  ...
  {
    string property = PropStripPrefix(propertyID, "");
    if ((property == "gameObjectName")  and  (value.GetType() == PropertyValue.INDEX_TYPE))
      m_gameObjectName = value.AsString();

    // this generally wouldn't be used as the type isn't really meant to be changed once the handler object
    // has been initialized, so it's here for completeness really
    else if ((property == "gameObjectType")  and  (value.GetType() == PropertyValue.INDEX_TYPE))
  ...
  public string[] GetPropertyElementList(string propertyID)
  {
    ...
    if (property == "gameObjectName")
    {
      ...
      // dw: this doesn't work as the driver characters don't really exist until Driver starts and runs the Driver Setup rule
/*      else if (gameObjectType == "drivercharacter")
      {
        DriverCharacter[] drivers = World.GetDriverCharacterList();
        for (i = 0; i < drivers.size(); i++)
          retStrList[i] = drivers[i].GetName();
      }*/
      else if (m_gameObjectType == "industry")
      {
      ...
      else if (m_gameObjectType == "cameratarget")
      {
        ...
        Interface.Log("# EnumeratedGameObjectRule.GetPropertyElementList> ERROR: Unknown game object type: '" + m_gameObjectType + "'");
  ...
  public void SetProperties(Soup soup)
  {
    ...
    // usually the type is initialized from Init() via the config file, only load from soup if it is not already set
