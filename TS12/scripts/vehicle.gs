...
//! Vehicle is a piece of rolling stock, or a locomotive.
//
// A vehicle is always part of a Train consist.  If a vehicle is decoupled and left alone, %Trainz
// will create a new consist for it.
//
...
//
// See Also:
//     Industry, Locomotive, ProductQueue, Train, Trackside
//
game class Vehicle isclass Trackside
{
	...
	//ProductFilter productRestrictionFilter; (Moved to native)
	//Browser info;
  ...
  {      
		...
		//AddHandler(me, "Browser-Closed", "", "CloseDetails");
	...
	//
	public bool GetFacingRelativeToTrain(void)
	{
  ...
  // Note: During multiplayer games this function will only succeed on the server
	...
	//! Sets the running number of this vehicle.
  //
  // Note: During multiplayer games this function will only succeed on the server and owning clients
	//
	// Param:  rn  Running number of this vehicle.  This should be limited to spaces and digits but
	//             letters are allowed just in case.
	//
	public native void SetRunningNumber(string rn);
  
	//! Gets the running number of this vehicle.
	//
	// Returns:
	//     Returns the running number of this vehicle.
	//
  ...
  public string GetDescriptionHTML(void)
  {
    ...
    KUID kuidToTrain = core.LookupKUIDTable("DuplicateToTrain");
    string imgToTrainString = kuidToTrain.GetHTMLString();
    
    string htmlString = /*"<html><body>"*/ "<font color=#FFFFFF>" + strTable.GetString("vehicle_propery_name") + ": <a href=\"live://property/runningnumber\">" + runningNumber + "</a><BR>";
		
		//
		// Train Priority on track
		//
		htmlString = htmlString + "<p>" + strTable.GetString("vehicle_train_priority") + ": <a href='live://property/priority-number'>" + (string)GetMyTrain().GetTrainPriorityNumber() + "</a></p>";
		...
		if(GetMyTrain().GetFrontmostLocomotive())
		{
			...
			//
			htmlString = htmlString + "<p>" + strTable.GetString("vehicle_train_headlight") + ": <a href='live://property/headlightstate'>" +  (string)GetMyTrain().GetHeadlightState() + "</a></p>";
			
			//
			// Vechicle HighBeam state
			//
			htmlString = htmlString + "<p>" + strTable.GetString("vehicle_train_highbeam") + ": <a href='live://property/highbeamstate'>" +  (string)GetMyTrain().GetHighBeams() + "</a></p>";
		}
		
		// "automatic-fireman"
		if(GetEngineType() == ENGINE_STEAM) {
			htmlString = htmlString + "<p>" + strTable.GetString("vehicle_train_automatic_fireman") + ": <a href='live://property/automatic-fireman'>" + (string)GetAutomaticFiremanState() + "</a></p>";
		}

    // Does this train have enough vehicles, as well as vehicles that support this command?
    ...
    if (refQueues.size() > 0)
    {
      for (i = 0; i < vehicles.size(); i++)
      {
        ...
        {
          // Another vehicle detected, yup we can enable the copy command ;)
          htmlString = htmlString + HTMLWindow.MakeLink("live://property/CopyToTrain/", HTMLWindow.MakeImage(imgToTrainString, true, 64, 64), strTable.GetString("vehicle_copy_to") ) + "<BR>";
          break;
        }
      }
    }
    ...
    if (queues.size() > 0)
    {
      ...
      int k = 0;
        ...
        //Interface.Log("Current Queue: " + queues[i].GetQueueName());
          ...
          for (k = 0; k < vehicleProductInfoCollection.ipicCollection.size(); k++)
                ...
                productList = productList + HTMLWindow.MakeCell(
								  HTMLWindow.MakeLink( "live://property/ToggleQueueDisplay/" + (string)k + "/" + (string)queueIndex, HTMLWindow.MakeImage( icon, true, 32, 32 ) )
                );
          ...
          // Now dump the pct bars
					for (k = 0; k < vehicleProductInfoCollection.ipicCollection.size(); k++)
					{
						if (vehicleProductInfoCollection.ipicCollection[k].GetProduct() == products[l])
						{
							...
							{
                productList = productList + HTMLWindow.MakeCell(
								  HTMLWindow.GetPercentBarCode(32, 4, HTMLWindow.GetQueueRoundedPercent(queues[i], asset))
                );
        ...
        //Interface.Log(productList);
				//if (productList == "")
				//	productList = "Queue is Empty";
				...
				for (k = 0; k < vehicleProductInfoCollection.ipicCollection.size(); k++)
				{
					...
					if (queueIndex > -1)
					{
						...
						{
							KUID kuid = vehicleProductInfoCollection.ipicCollection[k].GetProduct().GetKUID();
							string icon = kuid.GetHTMLString();

							string type = "";

							type = strTable.GetString("vehicle_units");
              /*
              if (vehicleProductInfoCollection.ipicCollection[k].displayType == vehicleProductInfoCollection.ipicCollection[k].DISPLAY_TYPE_MASS)
								type = "Tonnes";
							if (vehicleProductInfoCollection.ipicCollection[k].displayType == vehicleProductInfoCollection.ipicCollection[k].DISPLAY_TYPE_VOLUME)
								type = "Tonnes";
							if (vehicleProductInfoCollection.ipicCollection[k].displayType == vehicleProductInfoCollection.ipicCollection[k].DISPLAY_TYPE_COUNT)
								type = "Items";
              */
							...
							openProducts = openProducts + HTMLWindow.MakeTable(
																							HTMLWindow.MakeRow(
																								HTMLWindow.MakeCell( HTMLWindow.MakeImage( icon, true, 64, 64 ), "width=64" )
																								+ HTMLWindow.MakeCell( HTMLWindow.MakeBold( vehicleProductInfoCollection.ipicCollection[k].GetProduct().GetLocalisedName() ) + "<BR>" + HTMLWindow.MakeItalic(type + ": ") + HTMLWindow.MakeLink( "live://property/SetQueueProductCount/" + (string)k + "/" + (string)queueIndex, (string)count ) + "<BR>" + HTMLWindow.MakeLink( "live://property/RemoveQueueProduct/" + (string)k + "/" + (string)i, HTMLWindow.MakeImage(kuidImgRemoveString, true, 32, 32) ) )
																							)
																							, "bgcolor=#FFAD0059 width=400 bordercolor=#00000059"
																						);
							break;
						}
					}
				...
				KUID kuidArrowRight = core.LookupKUIDTable("imgArrowRight");
				string imgArrowRightString = kuidArrowRight.GetHTMLString();
				...
				for (k = 0; k < vehicleProductInfoCollection.ipicCollection.size(); k++)
				{
					...
					if (queueIndex > -1)
					{
						// Found a product in this queue.
						// Properties of the process list:
						htmlString = htmlString + HTMLWindow.MakeTable(
																				HTMLWindow.MakeRow(
																					HTMLWindow.MakeCell( HTMLWindow.MakeImage(imgArrowRightString, true, 16, 16) + " " + queues[i].GetQueueName(), "width=190" )
																					// Remove setting of queue size
                                          //+ HTMLWindow.MakeCell( HTMLWindow.MakeItalic( "Max Load: " ) + HTMLWindow.MakeLink( "live://property/SetQueueSize/" + (string)i, (string)queues[i].GetQueueSize() ) , "width=80")
																					+ HTMLWindow.MakeCell( HTMLWindow.MakeItalic( strTable.GetString("vehicle_max_load" )) + (string)queues[i].GetQueueSize(), "width=80")
																				, "bgcolor=#C9880059")
																				+ HTMLWindow.MakeRow(
																					HTMLWindow.MakeCell( productList, "colspan=4" )
																				)
																				+ HTMLWindow.MakeRow(
																					HTMLWindow.MakeCell( openProducts + "<BR>", "colspan=4" )
																				)
																			, "bgcolor=#FFAD0059 width=100% bordercolor=#00000059"
																			) 
																			+ HTMLWindow.MakeTable( HTMLWindow.MakeRow( HTMLWindow.MakeCell( "" ) ), "height=4" );
				...
				if (k >= vehicleProductInfoCollection.ipicCollection.size())
				{
						...
						htmlString = htmlString + HTMLWindow.MakeTable(
																				...
																				, "bgcolor=#C9880059")
																				+ HTMLWindow.MakeRow(
																					HTMLWindow.MakeCell( productList, "colspan=3" )
																				)
				...
				//htmlString = htmlString + HTMLWindow.EndTable();
				
			}
		}

    htmlString = htmlString + "</font>";//</body></html>";
    ...
    if (propertyID == "runningnumber")
	  ...
	  if (propertyID[0, 16] == "AddQueueProduct/")
	  {
		...
		if (propertyID[0, 21] == "SetQueueProductCount/")
		{
			...
			returnType = "int,0,999999999,10000";
		}
  ...
  void HideAllQueueItems(int productIndex, ProductQueue foundQueue)
  {
	...
	public string[] GetPropertyElementList(string propertyID)
	{
		...
		if (propertyID[0, 16] == "AddQueueProduct/")
		{
			...
			//retList = GetProductNameList(process.queue);
			...
			Asset[] assetList = pf.GetProducts();
          ...
          {
					  LinkedData ll = new LinkedData();
					  ll.text = assets[i].GetLocalisedName();
					  ll.index = i;
            linkedList.Add(ll);
          }
			}
			...
			// Copy the list of strings to the return list.
			for (i = 0; i < linkedList.data.size(); i++)
				retList[i] = linkedList.data[i].text;
	...
	void SetPropertyValue(string propertyID, string value, int index)
	{
		...
		{
			propertyID[0, 16] = null;
			Asset[] finalList = new Asset[0];
			linkedList.Init();


			// Add the selected product to the selected queue.
			Asset[] assets = World.GetAssetList("product");
					...
					if (!pf.DoesAcceptProduct(assets[i]))
          {
			...
			// Copy the list of strings to the return list.
			for (i = 0; i < linkedList.data.size(); i++)
				finalList[i] = cast <Asset> linkedList.data[i].gsobject;

			// list built, find it.
			Asset foundAsset = finalList[index];
			// update the industryProductInfoCollection (which will update the queue)
			vehicleProductInfoCollection.AddQueueToProduct(foundAsset, queue);

      int productIndex = vehicleProductInfoCollection.GetProductIndex(foundAsset);
      if (productIndex > -1)
      {
