...
//  [ "Browser-URL"     | URL    | browser object  | broadcast    ]
//  [ "Browser-Closed"  | null   | browser object  | broadcast    ]
//  [ "Browser-Button"  | button | browser object  | broadcast    ]}
//
...
final game class Browser isclass GameObject
{
	...
	//! \name   Browser Window Styles
	//  \anchor windowStyle
	//@{
	//! Different border states a browser can have.
	//
	// See Also:
	//     Browser::SetWindowStyle()
	//

	//! Browser without frame (i.e. not in a bordered window).
	public define int STYLE_NO_FRAME = 0;

	//! Default browser style.  This is defined by %Trainz itself and usually means the browser window will have a framed border if this style is chosen.
	public define int STYLE_DEFAULT = 1;

	//! Browser with no frame that the user can't interact with the window of.
	public define int STYLE_TOOLTIP = 2;

	//! Browser for use as a driver style HUD, has a light border that matches the existing driver HUDs.
	public define int STYLE_HUD_FRAME = 3;

	//! Similar to default but with a slimmer frame, no minimise/maximise/close buttons and no window title
	public define int STYLE_SLIM_FRAME = 4;

	//@}

  // Browser window button overlays, used for SetButtonOverlayStyle
  // Player clicks on these buttons will generate a "Browser-Button" message
  // with the 'minor' string noted beside the button definition. Disabled
  // buttons generate no messages.
  public define int BS_None             = 0;    // No button
  public define int BS_Left             = 1;    // Left arrow button (minor = "left-arrow")
  public define int BS_LeftDisabled     = 2;    // Disabled left arrow
  public define int BS_Right            = 3;    // Right arrow (minor = "right-arrow")
  public define int BS_RightDisabled    = 4;    // Disabled right arrow
  public define int BS_OK               = 5;    // OK button (tick) (minor = "ok")
  public define int BS_OKDisabled       = 6;    // Disabled OK
  public define int BS_Cancel           = 7;    // Cancel (cross) (minor = "cancel")

  // Browser window priorites
  ...
  public define int BP_Window           = 2;    // Window layer priority, use for normal windows
	...
	public native void LoadHTMLString(string htmlData);
	...
	//! Sets the style of window used by this browser (either with or without a frame).
	//
  ...
  //
  // Note: Window styles imply priority, if customising priority
...
//! %Interface that allows a browser to be created as well as basic screen controls.
//
// See Also:
//     Browser, Interface, Constructors::NewBrowser()
//
final static class BrowserInterface
{
	//! Deprecated, use Constructors::NewBrowser() instead.
	public native Browser CreateBrowser(void);

	//! Sets the color of an overlay and transitions to that overlay over the given time.
	//
	// Param:  red        Red value of overlay color.
	// Param:  green      Green value of overlay color.
	// Param:  blue       Blue value of overlay color.
	// Param:  alpha      Alpha setting of overlay (transparency).
	// Param:  morphTime  Duration of fading to the new overlay.
	//
	public native void SetOverlayColor(float red, float green, float blue, float alpha, float morphTime);

	//! OBSOLETE - Save support is mandatory.
	public void SetSaveAllowed(bool saveAllowed) {}

	//! Fades the screen out and back in again according to the given color and time parameters.
	//
	// Param:  runningThread  GameObject where thread that called this method is.
	// Param:  red            Red value of color to fade out to.
	// Param:  green          Green value of color to fade out to.
	// Param:  blue           Blue value of color to fade out to.
	// Param:  alpha          Alpha value of layer faded to (transparency).
	// Param:  time           Duration of fading.  This value will be split equally into three
	//                        sections for fade out, idle and fade in.
	//
  ...
  //! Masks obscene language in the string passed
  //
  //  Searches the string passed for obscene langauge and replaces it with a
  //  string of non-alphanumeric characters ($@!). It is good practice to run 
  //  this function over any player entered text that is visible to other 
  //  players. For example, chat text, multiplayer game listings, etc.
  //
