...
class IndustryTrainController isclass GSObject
{
  ...
  public void SetProperties(Soup soup)
  {
    ...
    Soup vehicleSoup = soup.GetNamedSoup("vehicles");
    for (i = 0; i < vehicleSoup.CountTags(); ++i)
    {
      string vehicleName = vehicleSoup.GetNamedTag((string)i);
      Vehicle vehicle = null;

      int v;
      for (v = 0; v < vehicles.size() and !vehicle; ++v)
        if (vehicles[v].GetName() == vehicleName)
          vehicle = vehicles[v];

      if (vehicle)
        AddVehicle(vehicle, false);
      else
        Interface.Log("IndustryTrainController.SetProperties> Cannot find vehicle '" + vehicleName + "'");
    }
  }
  ...
  public Soup GetProperties(void)
  {
    ...
    if (m_vehicles)
    {
      ...
      for (i = 0; i < m_vehicles.size(); ++i)
        vehicleSoup.SetNamedTag((string)i, m_vehicles[i].GetName());
      soup.SetNamedSoup("vehicles", vehicleSoup);
    }
