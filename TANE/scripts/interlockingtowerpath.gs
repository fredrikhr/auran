...
class InterlockingTowerPath isclass GameObject
{
  ...
  public mandatory void SetProperties(SecurityToken token, Soup properties);
  ...
  public mandatory void Init(InterlockingTower tower, SecurityToken token)
  {
    ...
    StringTable coreStrings = Constructors.GetTrainzAsset().GetStringTable();
  ...
  public thread void SetObjectsOwned(SecurityToken token, bool bTakeOwnership)
  {
    if (bTakeOwnership and (!m_itpWantObjectOwnership or World.GetCurrentModule() != World.DRIVER_MODULE))
      return;
  ...
  thread void AttemptPathActivation()
  {
    ...
    {
      DebugLog("InterlockingTowerPath.AttemptPathActivation> Invalid state", 1);
      return;
    }
  ...
  thread void AttemptPathCancellation()
  {
    ...
    if (isPathClear)
    {
      ...
      {
        m_itpPathState = ITP_PATH_NOT_SET;
        PostMessage(me, "InterlockingTowerPath", "Cancelled", 0.f);
  ...
  public mandatory Soup GetProperties(SecurityToken token)
  {
    ...
    for (i = 0; i < m_itpTrains.size(); ++i)
    {
      ...
      trainData.SetNamedTag("train", m_itpTrains[i].m_trainID);
  ...
  //=============================================================================
  obsolete void LegacyLoadPathObjectFromName(InterlockingObject pathObject, string name)
  {
  ...
  // Parm: token - A token from the owning tower with rights "path-control"
  //=============================================================================
  public mandatory void SetProperties(SecurityToken token, Soup parentProperties)
  {
    if (!m_itpParentTower.ValidateToken(token, "path-control"))
    {
