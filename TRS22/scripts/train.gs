...
//
// Messages posted by/to this class:
...
//  [ "Train"     | "AI control mode changed"     | train     | train         ]
...
final game class Train isclass GameObject
{
  ...
  //=============================================================================
  // Name: SetBail
  // Desc: Sets the bail control as depressed for a short period.
  //=============================================================================
  ...
  // Note: A "Train","BrakeLightChanged" message is sent when this value changes.
  //=============================================================================
  ...
  //=============================================================================
  // Name: TrackSearch
  // Desc: Searches the train from the current train position for the passed
  //       direction and distance, returning an array of all found objects. This
  //       tracksearch will follow the currently set junction directions, and
  //       terminate at the end of track (or possibly any impassable junctions).
  // Parm: dir - The Direction to search. Use true to search forward or false to
  //       search backward. Note that the train's direction is conceptual and
  //       may not match what the visual facing of the traincar objects.
  // Parm: distance - The distance to search. If searching backwards (dir=false)
  //       the length of the train will be added to this value.
  // Retn: MapObject[] - An array of every track object and industry found along
  //       the track in the direction specific.
  //=============================================================================
  public native MapObject[] TrackSearch(bool dir, float distance);


  //=============================================================================
  // Name: GetTrackmarkDirection
  // Desc: Gets the direction of the given track mark relative to this train.
  // Retn: int - One of the results defined on this class (e.g. RESULT_FORWARD)
  //       indicating where the trackmark is in relation to this train.
  //=============================================================================
  ...
  //=============================================================================
  // Name: Derail
  // Desc: Forcibly derails this train.
  // Note: During multiplayer games this function will only succeed on the server
  //       and owning clients.
  //=============================================================================
  ...
  //=============================================================================
  // Name: Turnaround
  // Desc: Reverse the conceptual direction of this train (i.e. forward becomes
  //       backwards and backwards becomes forward).
  // Note: The 3D vehicle objects do not turnaround, just the concept of forward
  //       for the throttle/reverser are affected.
  // Note: During multiplayer games this function will only succeed on the server
  //       and owning clients.
  //=============================================================================
  ...
  //=============================================================================
  // Name: GetSpeedLimit
  // Desc: Gets the posted speed limit for this train. The speed limit is
  //       dependent on the trackside objects placed in Surveyor and the facing
  //       direction of the train. A default value of 40mph applies to track that
  //       isn't sign posted.
  //=============================================================================
  ...
  //=============================================================================
  // Name: SetFloatingLimit
  // Desc: Sets the floating speed limit level above the sign posted limit. The
  //       floating limit is an amount of tolerance above the current speed
  //       limit. Messages for floating speed violations are not to be confused
  //       with regular speed limit messages.
  // Parm: delta - The amount above the posted speed limit at which the floating
  //       speed limit messages should be posted.
  // Parm: enable - Whether to post the floating limit messages for this train.
  //
  // Mesg: "Train","Over floating speed limit" - Posted from this train when it's
  //       speed exceeds the posted limit plus the set floating limit delta.
  // Mesg: "Train","Under floating speed limit" - Posted from this train when it's
  //       speed was over the posted limit plus the set floating limit delta, and
  //       now isn't. This includes if the floating limit has been disabled while
  //       this train was over it.
  // Note: During multiplayer games this function will only succeed on the server
  //       and owning clients.
  //=============================================================================
  ...
  //=============================================================================
  // Name: IsOverFloatingLimit
  // Desc: Returns whether this train is currently considered (by native code) to
  //       be over the floating speed limit. Calling this is preferable to
  //       testing the speed manually, as it's guaranteed to match the current
  //       native code state, and avoid the possibility of desync with the posted
  //       "over/under floating speed limit" messages.
  // Retn: bool - whether native code currently considers this train to be over
  //       the floating speed limit.
  //=============================================================================
  public native bool IsOverFloatingLimit();


  //=============================================================================
  // Name: SetAdvisoryLimit
  // Desc: Set the speed limit for a script controlled train.
  // Parm: speed - The advisory limit to set (in m/s), or 0 for no limit.
  // Note: During multiplayer games this function will only succeed on the server
  //       and owning clients.
  //=============================================================================
  ...
  //=============================================================================
  // Name: GetAdvisoryLimit
  // Desc: Gets the current script-set advisory speed limit (in m/s).
  //=============================================================================
  public native float GetAdvisoryLimit(void);


  //=============================================================================
  // Name: MarkAdvisoryLimit
  // Desc: Temporarily records the currently set advisory speed limit. This
  //       recorded limit can then be restored by calling RevertAdvisoryLimit().
  //       If an advisory limit is already recorded from a past call to this
  //       function, then the second call will be silently ignored.
  //       If this is called on a train that has no advisory limit set, then that
  //       state is still recorded, and the next call to RevertAdvisoryLimit()
  //       will restore to that state.
  // Note: During multiplayer games this function will only succeed on the server
  //       and owning clients.
  //=============================================================================
  ...
  //=============================================================================
  // Name: RevertAdvisoryLimit
  // Desc: Reverts the advisory speed limit for this train to that recorded by a
  //       previous call to MarkAdvisoryLimit(). If no previous advisory limit is
  //       recorded, this call will be silently ignored.
  // Note: During multiplayer games this function will only succeed on the server
  //       and owning clients.
  //=============================================================================
  ...
  //=============================================================================
  // Name: SetAdvisoryLimit2
  // Desc: Similar to SetAdvisoryLimit, but for custom script uses. Has no impact
  //       on core Trainz gameplay.
  // Parm: speed - The advisory limit to set (in m/s), or 0 for no limit.
  // Note: During multiplayer games this function will only succeed on the server
  //       and owning clients.
  //=============================================================================
  ...
  //=============================================================================
  // Name: GetAdvisoryLimit2
  // Desc: Gets the current script-set custom advisory speed limit (in m/s).
  //=============================================================================
  ...
  //       message when all of the thresholds are no longer exceeded. No
  //       tolerance is permitted for exceeding the limit, however a small
  //       tolerance may be allowed (either temporal or actual) for returning
  ...
  // 
  public void RunSchedule(Schedule schedule, bool loop, float delay);
  ...
  //
  public legacy_compatibility void UpdateScheduleWindow(string vehicleName);
  ...
  legacy_compatibility bool CommandOnAnnouncement(string state);
...
*/
  ...
  public thread legacy_compatibility void ThreadRunSchedule(Schedule schedule, bool loop, float delay)
  {
    if (!m_schedule or !m_schedule.commands)
    {
      Interface.Log("Train.RunSchedule> Unable to start schedule on train " + GetDebugName() + " - empty schedule");
  ...
  run:
    ...
    for (m_scheduleIndex = 0; m_scheduleIndex < m_schedule.commands.size(); ++m_scheduleIndex)
    {
      ...
      if (cmd.objID and (!cmd.obj or !Router.DoesGameObjectStillExist(cast<GameObject>(cmd.obj))))
      {
        cmd.obj = World.SynchronouslyLoadGameObjectByID(cmd.objID);
        if (!cmd.obj)
          Interface.Exception("Train.ThreadRunSchedule> Failed to (re)load object: " + cmd.objID.GetDebugString());
      }
      ...
      switch (cmd.command)
      {
        case Schedule.CMD_TAKE_JUNCTION:
        {
          ...
          if (!junction)
          {
            // If we had to reload the object, this param may actually be a SceneryWithTrack
            SceneryWithTrack swt = cast<SceneryWithTrack>(cmd.obj);
            if (swt and cmd.cb >= 0 and cmd.cb < swt.GetAttachedJunctions().size())
            {
              junction = swt.GetAttachedJunctions()[cmd.cb];
              cmd.obj = junction;
            }
            else
            {
              Interface.Log(logMsg + "CMD_TAKE_JUNCTION: Junction missing/invalid");
              goto done;
            }
          }
    ...
    SetAutopilotTarget(cast<Trackside>(null));
  ...
  public void RunSchedule(Schedule schedule, bool loop, float delay)
  {
    ...
    if (m_schedule  or  m_scheduleRunning)
    {
      Exception("Train.RunSchedule> schedule already running for train " + GetDebugName());
      return;
    }
    ...
    ThreadRunSchedule(schedule, loop, delay);
  }
  ...
  void JunctionInnerEnterHandler(Message msg)
  {
    ...
    //
    Permit junctionPermit = junction.RequestPermit(null, me, junction.GetDirection());
    if (junctionPermit)
  ...
  void PermitReleaser(Message msg)
  {
    ...
    if (junction and m_junctionPermits and m_junctionPermits.size() > 0)
    {
      ...
      {
        // Remove our oldest permits, and decrement the permit count.
      ...
      {
        // Something funny could be going on, see if we've missed a message.
        int i = 0;
  ...
  void ReleaseAllPermits()
  {
    ...
    if (m_junctionPermits)
		{
			...
			Interface.Log("Train.RetestAllPermits> " + (string)i + " permits (train " + GetTrainDisplayName() + ")");
    ...
    {
			Interface.Log("Train.RetestAllPermits> no junction permits (train " + GetTrainDisplayName() + ")");
    }
  ...
  //=============================================================================
  // Name: MatchExistingJunctionPermit
  // Desc: 
  //=============================================================================
  ...
  //=============================================================================
  // Name: RecordTrackPermit
  // Desc: Record the new permit into our track-permit list. The new permit
  //       should not already exist in the list.
  //=============================================================================
  void RecordTrackPermit(Permit newPermit)
  {
    if (!newPermit)
  ...
  //=============================================================================
  // Name: MatchExistingTrackPermit
  // Desc: Find any existing track permit that matches the new permit.
  //=============================================================================
  Permit MatchExistingTrackPermit(Permit newPermit)
  {
    if (!m_trackPermits)
  ...
  //=============================================================================
  // Desc: Internal helper for use by MatchBlockingTrackPermit(). Do not call
  //       this directly, just use MatchBlockingTrackPermit().
  //=============================================================================
  native Permit MatchBlockingTrackPermitInternal(Permit newPermit);


  //=============================================================================
  // Name: MatchBlockingTrackPermit
  // Desc: Find any existing track permit that blocks (or matches) the new permit.
  //=============================================================================
  Permit MatchBlockingTrackPermit(Permit newPermit)
  {
    ...
    // During multiplayer script may not have a complete picture of all the
    // permits for other trains, but native code does. Check there too.
    return MatchBlockingTrackPermitInternal(newPermit);
  }


  //=============================================================================
  // Name: PromoteTrackPermit
  // Desc: Promote an existing track permit to the end of the track-permit list.
  //=============================================================================
  ...
  //=============================================================================
  // Name: TrackPermitReleaser
  // Desc: Handler for "Train", "LeaveTrack" and etc.
  //=============================================================================
  void TrackPermitReleaser(Message msg)
  {
    if (!m_trackPermits)
    ...
    for (i = 0; i < m_trackPermits.size(); i++)
    {
      ...
      {
        Interface.Log("Train.TrackPermitRelease> Released permit for: " + obj.GetDebugName());
        RemoveTrackPermit(i);
...
/*
  ...
  Permit GetJunctionPermit(JunctionPermitInfo info)
  {
    ...
    if (info.direction == JunctionBase.DIRECTION_BACKWARD  or  info.direction == JunctionBase.DIRECTION_NONE)
    {
      ...
      junctionPermit = info.junction.RequestPermit(null, me, info.junction.GetDirection());
    }
    else
    {
  ...
  retrySwitchForTrain:
	...
	bool CommandNavigateTrack(Track track)
	{
		...
		AITrainScope scope = OpenAITrainScope();
		PushScheduleState(scope, SS_TRACK, "");
		m_scheduleDrivingLocked ++;
  ...
  {
    AITrainScope scope = OpenAITrainScope();
    PushScheduleState(scope, SS_STOP, "");
  ...
  void LogCommandCompleteJunctionsList(string debugString, GameObjectID[] junctionIDs)
  {
	...
	bool AITrainStoppedMoving(bool force)
	{
		JunctionBase junctionBase = GetAutopilotStopJunctionBase();
		if (!junctionBase)
		{
		...
		// Check if we have a permit for this function (poorly named function)
		if (IsStillInJunction(junctionBase))
		{
			Interface.Log("Train.AITrainStoppedMoving> Already hold permit for autopilot stop " + junctionBase.GetDebugString());
			...
			if (junctionBase.GetJunctionOwner())
			{
				Interface.Log("Train.CommandNavigateJunction> Junction is owned and cannot be altered.");
				PostMessage(junctionBase.GetJunctionOwner(), "Schedule", "Blocked", 0);
				return false;
			}

			int direction = junctionBase.GetDirectionToTrain(me);
			if (direction == Junction.DIRECTION_BACKWARD)
				direction = junctionBase.GetDirection();
			else
				direction = Junction.DIRECTION_BACKWARD;

			if (!junctionBase.SwitchForTrain(null, me, direction))
				return false;
		...
		Interface.Log("Train.AITrainStoppedMoving> Navigating " + junctionBase.GetDebugString());
		...
		if (!CommandNavigateJunction(junctionBase, direction, true, true))
			return false;
      ...
      if (aiTrainStuck)
      {
        if (GetAutopilotMode() == CONTROL_AUTOPILOT)
        {
          ...
          if (scheduleTimeout > 3)
          {
            ...
            if (m_schedule)
            {
              // Abandon autopilot mode, with feedback
              StopSchedule();
              Interface.Print(strTable.GetString1("interface-train-autopilot-abort-stuck", GetTrainDisplayName()));
            }
          }
          else if (bDidTimeOut)
          {
            // Wait a bit longer each time
            timer = timer * scheduleTimeout;
...
fail_ccj:
  ...
  bool CommandCoupleVehicle(Vehicle vehicle)
  {
    // Early out if the vehicle is already coupled here
    ...
    // Attempt to couple at both ends of each train. If any of the vehicles are
    // close enough together then this will finalise the operation.
    ...
    wait()
    {
      ...
      Train_StoppedMoving:;
      {
        ...
        Train autopilotStopTrain = GetAutopilotStopTrain();
        if (autopilotStopTrain and autopilotStopTrain.ContainsVehicle(vehicle))
        {
          // We know this is going to fail if we're in multiplayer and the trains are
          // owned by different players, so don't even attempt it.
          if (MultiplayerGame.IsActive())
          {
            DriverCharacter driver1 = GetActiveDriver();
            DriverCharacter driver2 = vehicle.GetMyTrain().GetActiveDriver();
            if (driver1 and driver2 and driver1.GetOwnerPlayer() != driver2.GetOwnerPlayer())
            {
              Interface.Log("Train.CommandCoupleVehicle> Cannot couple to another players train");
              BeginAITrainStuck(scope);
              continue;
            }
          }

          // We've stopped because of a train, but that train contains our
          // target vehicle, so just drive into it.
          SetAutopilotMode(Train.CONTROL_SCRIPT);
          SetDCCThrottle(Math.Sqrt(2.3 / GetAITrainMaxSpeed()));
          continue;
        }

        // We've stopped, but it's not for the reason we want
        Interface.Log("Train.CommandCoupleVehicle> Blocked by another train, cannot proceed");
        BeginAITrainStuck(scope);
        //PostMessage(me, "Train", "CommandCoupleVehicleTimer", 2.0f);
        continue;
      }
      ...
      on "Vehicle", "BadCouple", msg:
        // Treat this the same as a successful couple
        goto Vehicle_Coupled;
      ...
      Vehicle_Coupled:;
      {
        ...
        {
          // This train now contains the target vehicle, we're done
        ...
        // Ignore the message if it's from some vehicle not coupled here
        if (!ContainsVehicle(cast<Vehicle>(msg.src)))
          continue;

        // We don't have the target vehicle but we did couple to something, what was it?
      ...
      on "Schedule", "Abort":
        Interface.Log("Train.CommandCoupleVehicle> Schedule, Abort");
        PopScheduleState(scope);
        return false;
    };
    ...
    if (!force)
    {
			...
			if (permits.size())
			{
				...
				AITrainScope scope = OpenAITrainScope();
				PushScheduleState(scope, SS_TURNAROUND, "");
  ...
  //=============================================================================
  // Name: ToggleScheduleWindow
  // Desc: Toggles the schedule window for this train.
  //=============================================================================
  ...
  //=============================================================================
  // Name: IsScheduleWindowVisible
  // Desc: Determines if the schedule window of this train is visible.
  //=============================================================================
  ...
  //=============================================================================
  // Name: UpdateScheduleWindow
  // Desc: Updates any active schedule window
  //=============================================================================
  public legacy_compatibility void UpdateScheduleWindow(string vehicleName)
  {
