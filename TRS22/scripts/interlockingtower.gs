...
class InterlockingTrain
{
  ...
  //=============================================================================
  legacy_compatibility GameObjectID LegacyGetGameObjectIDFromName(string name)
  {
  ...
  //=============================================================================
  legacy_compatibility void CacheObsoleteVariables(void)
  {
...
//       If you are extending this script do NOT use this namespace, as it may
//       cause future code additions to break your tower script.
//=============================================================================
game class InterlockingTower isclass MapObject
{
  ...
  bool                      m_itHasDriverLoad;  // Indicates the tower has been loaded in Driver
  ...
  public void Init(Asset asset)
  {
    ...
    AddHandler(Interface, "World", "ModuleInit", "OnModuleInit");
  ...
  //=============================================================================
  legacy_compatibility void CreatePathRequeryNavPoint(Signal entrySignal, bool bIsLegacyLoad)
  {
    if (!entrySignal)
      return;
  ...
  void OnPathCancelled(Message msg)
  {
    ...
    if (!srcPath or srcPath.GetOwningTower() != me)
      return;
  ...
  mandatory void OnModuleInit(Message msg)
  {
    if (World.GetCurrentModule() == World.DRIVER_MODULE)
      m_itHasDriverLoad = true;

    if (m_itHasDriverLoad)
    {
      SetPathObjectsOwned(true);
  ...
  //=============================================================================
  // Name: ReleaseOwnedObjectsNoLongerReferenced
  // Desc: Iterates the passed object array and releases ownership on any object
  //       that is longer referenced by this tower. Called from the end of
  //       SetProperties() to ensure we don't leak ownership on anything.
  // Parm: objects - The array of objects validate ownership for.
  //=============================================================================
  thread void ReleaseOwnedObjectsNoLongerReferenced(InterlockingObject[] objects)
  {
    int oi;
    for (oi = 0; oi < objects.size(); )
    {
      // Get the map object. This may cause it to load from disk etc but we need
      // it for every part of this process.
      MapObject mapObj = objects[oi].GetMapObject(me);
      if (!mapObj)
        continue;

      bool bIsInAnyPath = false;
      int pi;
      for (pi = 0; !bIsInAnyPath and pi < m_itPaths.size(); ++pi)
        bIsInAnyPath = m_itPaths[pi].IsObjectInPathDefinition(mapObj, objects[oi].m_childIndex, true);

      // If the object is referenced then remove it from the array, otherwise
      // leave it there and move on to the next one.
      if (bIsInAnyPath)
        objects[oi, oi + 1] = null;
      else
        ++oi;

    } // for (oi = 0; oi < objects.size(); )

    // Any objects that are still in the array need to be released.
    if (objects.size() > 0)
    {
      // Ownership changes are quite complicated as they require handling all the
      // different object types, so create a path object to do the work for us.
      InterlockingTowerPath path = CreateNewPath();
      path.SetPathName("ReleaseOwnedObjectsNoLongerReferenced");
      path.SetPathDefinition(objects);
      path.SetObjectsOwned(m_itTowerToken, false);
    }
  ...
  //=============================================================================
  // Name: MergeInterlockingObjectArrays
  // Desc: Merges two arrays of type InterlockingObject, with duplication checks.
  // Parm: mergeTo - The array to merge into. This will contain the unique object
  //       entries from both arrays after the function returns.
  // Parm: mergeFrom - The array to merge from. This will be empty after this
  //       function returns.
  //=============================================================================
  void MergeInterlockingObjectArrays(InterlockingObject[] mergeTo, InterlockingObject[] mergeFrom)
  {
    while (mergeFrom.size() > 0)
    {
      int i;
      for (i = 0; i < mergeTo.size(); ++i)
      {
        if (mergeTo[i].DoesMatchObject(mergeFrom[0]))
          break;
      }

      // If we didn't find a matching object, then add it.
      if (i >= mergeTo.size())
        mergeTo[mergeTo.size()] = mergeFrom[0];
  ...
  //=============================================================================
  // Name: UpdateMonitoredTrainsList
  // Desc: Helper function called from SetProperties to update the monitored
  //       trains array with new data (including any necessary sniffs etc).
  //=============================================================================
  void UpdateMonitoredTrainsList(InterlockingTrain[] newItTrains)
  {
    int oi, ni;

    // Iterate the existing array to check for removals first.
    for (oi = 0; oi < m_itTrains.size(); )
    {
      for (ni = 0; ni < newItTrains.size(); ++ni)
      {
        if (m_itTrains[oi].m_itTrainID.DoesMatch(newItTrains[ni].m_itTrainID))
          break;
      }

      if (ni >= newItTrains.size())
      {
        DebugLog("InterlockingTower.UpdateMonitoredTrainsList> Removing train entry (" + m_itTrains[oi].m_itTrainID.GetDebugString() + ")", 2);
        Sniff(m_itTrains[oi].m_itTrainID, "Train", "StoppedMoving", false);
        Sniff(m_itTrains[oi].m_itTrainID, "Train", "Cleanup", false);

        m_itTrains[oi, oi + 1] = null;
      }
      else
      {
        // A match was found, keep checking.
        ++oi;
      }
    }

    // Then process any updates or additions.
    for (ni = 0; ni < newItTrains.size(); ++ni)
    {
      for (oi = 0; oi < m_itTrains.size(); ++oi)
      {
        if (newItTrains[ni].m_itTrainID.DoesMatch(m_itTrains[oi].m_itTrainID))
        {
          // This train matches, since we're already monitoring it we just need
          // to swap the new data in.
          DebugLog("InterlockingTower.UpdateMonitoredTrainsList> Updating train entry (" + m_itTrains[oi].m_itTrainID.GetDebugString() + ")", 4);
          m_itTrains[oi] = newItTrains[ni];
          break;
        }
      }

      if (oi >= m_itTrains.size())
      {
        // The train wasn't found, so add a new entry and sniff it by ID.
        DebugLog("InterlockingTower.UpdateMonitoredTrainsList> Adding train entry (" + newItTrains[ni].m_itTrainID.GetDebugString() + ")", 4);
        m_itTrains[m_itTrains.size()] = newItTrains[ni];

        Sniff(newItTrains[ni].m_itTrainID, "Train", "StoppedMoving", true);
        Sniff(newItTrains[ni].m_itTrainID, "Train", "Cleanup", true);
      }
    }
  ...
  //=============================================================================
  thread legacy_compatibility void UpdateLegacyNamedNavPointSets(int dataVersion)
  {
    ...
    for (i = 0; i < m_itNavPoints.size(); ++i)
    {
      ...
      if (m_itNavPointIDs[i])
        CreatePathRequeryNavPoint(cast<Signal>(World.GetGameObjectByIDIfLoaded(m_itNavPointIDs[i])), true);
    }
  }
  ...
  public mandatory Soup GetProperties()
  {
    Soup parentProperties = inherited();
    ...
    properties.SetNamedTag("has-driver-load", m_itHasDriverLoad);
  ...
  //=============================================================================
  // Name: SetProperties
  // Desc: Sets this tower's internal state using the Soup passed.
  //=============================================================================
  public legacy_compatibility mandatory void SetProperties(Soup parentProperties)
  {
    inherited(parentProperties);

    // We need to make sure we don't leak owned objects, but just releasing our
    // ownership here would cause issues with any active paths. Instead we need
    // to record every owned object across all paths, with their return state,
    // and then process this list after SetProperties completes.
    InterlockingObject[] allPathObjects = new InterlockingObject[0];
    {
      int i;
      for (i = 0; i < m_itPaths.size(); ++i)
      {
        MergeInterlockingObjectArrays(allPathObjects, m_itPaths[i].GetPathDefinitionCopy());
        MergeInterlockingObjectArrays(allPathObjects, m_itPaths[i].GetExternalRequirementsCopy());
      }
    }
    ...
    InterlockingTrain[] newItTrains = new InterlockingTrain[trains.CountTags()];
    for (i = 0; i < trains.CountTags(); ++i)
    {
      newItTrains[i] = new InterlockingTrain();
      newItTrains[i].SetProperties(trains.GetNamedSoup(trains.GetIndexedTagName(i)));
    }
    UpdateMonitoredTrainsList(newItTrains);
    ...
    m_itHasDriverLoad = properties.GetNamedTagAsBool("has-driver-load", m_itHasDriverLoad);
    ...
    // Release ownership on any objects that are longer referenced
    ReleaseOwnedObjectsNoLongerReferenced(allPathObjects);
