...
game class Vehicle isclass Trackside
{
  ...
  // 
  Library GetDriverAchievementsLibrary()
  {
    if (!m_DAULib)
    {
  ...
  public bool LoadProduct(LoadingReport report)
  {
    ...
    // Test if the loading operation would achieve anything. (We do this because
    // we don't want to start pfx, etc if there is no loading happening.)
    if (!CanLoadProductFromQueue(report.srcQueue))
      return false;

    // Begin Loading
    float vehicleMinTime = BeginLoad(report);
    float industryMinTime = industry.BeginLoad(report);
    bool bHasInterruptedLoad = !GetMyTrain().InterruptableLoadingSleep(Math.Fmax(vehicleMinTime, industryMinTime));
    ...
    if (amountLoaded > 0)
    {
      ...
      Library achievementsUpdater = GetDriverAchievementsLibrary();
      if (achievementsUpdater)
      {
        GSObject[] GSObjectArray = new GSObject[0];
        GSObjectArray[0] = report;
        achievementsUpdater.LibraryCall("recordIndustryLoadUnload", null, GSObjectArray);
      }
    }

    SetHasLoadedThisStop(true);
    return !bHasInterruptedLoad;
  }
