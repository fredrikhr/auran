...
class NavPoint
{
  ...
  public KUID         kuid;         // The asset to use for the nav point icon
  public int          icon = 0;     // Index of the texture to use within the specified asset
...
final static class NavPoints isclass GameObject
{
  ...
  public define int NAVICON_MAX = 27; // Identifier only, for validation.
  ...
  public define int NAVSTLYE_MAX = 3; // Identifier only, for validation.
  ...
  public define int NAVMODE_MAX = 3;  // Identifier only, for validation.
  ...
  public define int NAVTARGET_MAX = 2;  // Identifier only, for validation.
  ...
  public legacy_compatibility bool CreateNavigationPointSet(string id, int mode, NavPoint[] points, GameObjectID[] trainFilter, int drawDist);
  ...
  //===========================================================================
  native bool CreateNavigationPointSetInternal(string id, int style, Soup points, GameObjectID[] trainFilter, int drawDist);
  ...
  // Retn: bool - true on success, false on failure
  //===========================================================================
  public legacy_compatibility bool CreateNavigationPointSet(string id, int style, NavPoint[] points, GameObjectID[] trainFilter, int drawDist)
  {
    // Note: legacy_compatibility for fallback access to 'tMark'.
    ...
    {
      if (points[i].icon < 0 or (!points[i].kuid and points[i].icon >= NAVICON_MAX))
      {
        Interface.Exception("HTMLWindow.CreateNavigationPointSet> Invalid icon texture index");
        return false;
      }
      ...
      data.SetNamedTag("icon-kuid", points[i].kuid);
      data.SetNamedTag("icon-index", points[i].icon);
    ...
    return CreateNavigationPointSetInternal(id, style, nativePts, trainFilter, drawDist);
  }
  ...
  {
    return CreateNavigationPointSet(id, style, points, trainFilter, 0);
  }
  ...
  {
    return CreateNavigationPointSet(id, style, points, new GameObjectID[0], 0);
  }
